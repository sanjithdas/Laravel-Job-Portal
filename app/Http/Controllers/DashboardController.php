...
class DashboardController extends Controller
{
	...
	public function getAllJobs()
	{
		...
		$jobs = Job::latest()->orderBy('created_at', 'desc')->paginate(40);
		return view('admin.job', compact('jobs'));
		$data = [
			'foo' => 'bar'
		];
		$pdf = PDF::loadView('admin.job', $data)->download('job.pdf');
	}
	...
	public function usersApplied($id)
	{
		$job = Job::findOrFail($id);

		return view('admin.userapplied')->with('job', $job);
	}
}
